...
static void flush_quarantine(struct kmem_cache *s)
{
	...
	for_each_kmem_cache_node(s, node, n) {
		...
		list_for_each_entry(page, &n->full, lru)
			__flush_quarantine(s, n, page);
		/*
		 * If quarantine isn't enabled from the begining some pages
		 * might be unreacheable from here. n->quarantine.nr_objects
		 * cannot reach zero in this case.
		 */
...
{
	/* Drop quarantine left after flush_quarantine() */
	page->quarantine = NULL;
	dec_slabs_node(s, page_to_nid(page), page->objects);
	free_slab(s, page);
}
